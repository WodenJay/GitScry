...
pub(crate) struct PreparedCache {
    ...
    pub(crate) _lock: cache::SharedLock,
}
...
pub(crate) fn prepare_cache(repository: &git::Repository) -> Result<PreparedCache, AppError> {
    ...
    let shared = cache::acquire_shared(&repository.root, &mut progress)?;
    ...
    match plan {
        ...
        Plan::Incremental {
            ...
            if state.shallow_boundaries != expected.shallow_boundaries {
                refresh.extend(cache::boundary_refreshes(&repository.root)?);
            }
            refresh.extend(cache::commits_for_objects(
                &repository.root,
                &newly_available,
            )?);
            refresh.sort();
            ...
            cache::append(&repository.root, &snapshot)?
        }
    ...
    let shared = cache::acquire_shared(&repository.root, &mut progress)?;
    let state = match cache::inspect(&repository.root) {
        cache::Inspection::Ready(state)
...
fn evaluate(
    ...
    let cache::Inspection::Ready(state) = inspection else {
        return Ok(Plan::Rebuild {
            damaged: matches!(inspection, cache::Inspection::Damaged),
        });
    };
...
fn add_warnings(progress: &mut Vec<String>, expected: &Expected, missing_objects: &[String]) {
    if let Some(warning) = cache::shallow_warning(&expected.shallow_boundaries)
        && !progress.contains(&warning)
    {
        progress.push(warning);
    }
    if let Some(warning) = cache::missing_warning(missing_objects)
        && !progress.contains(&warning)
    {
        progress.push(warning);
    }
